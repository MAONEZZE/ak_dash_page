...
const TAMANHO_AVATAR: Record<number, number> = { 1: 54, 2: 42, 3: 42 };
...
function Degrau({ pessoa, posicao }: { pessoa: PessoaGeral; posicao: number }) {
  return (
    <div className={`flex flex-1 flex-col items-center gap-1 rounded-xl px-2 text-center ${ORDEM_VISUAL[posicao]} ${ALTURA_BASE[posicao]}`}>
      <span className="font-display text-[13px] font-extrabold text-accent-fg">{posicao}º</span>
      <Avatar nome={pessoa.rotulo} imagemUrl={pessoa.imagem_url} tamanho={TAMANHO_AVATAR[posicao]} />
      <span className="line-clamp-1 text-[14px] font-bold tracking-tight">{pessoa.rotulo}</span>
      <span className="text-[12.5px] font-semibold text-fg/50">{pessoa.pontuacao?.toFixed(0)} pts</span>
    </div>
  );
}
...
export function RankingPodio({ titulo, pessoas }: Props) {
  ...
  return (
    <article className="glass-panel flex flex-col gap-3 rounded-2xl p-3.5 h-full ">
      <span className="text-[12px] font-semibold uppercase tracking-[0.13em] text-fg/56">{titulo}</span>
      {ranking.length === 0 ? (
        <p className="text-[13.5px] text-fg/50">Cadastre as metas em dash.metricas_metas pra ver o ranking.</p>
      ) : (
