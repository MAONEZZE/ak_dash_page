    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-[15px] font-bold tracking-tight">{nome}</span>
        ...
        {destaque.map((m) => {
          ...
          return (
            <div key={m.metrica} className="flex flex-col gap-1">
              <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-fg/50">{m.nome_exibicao}</span>
              <span
                className={`font-display text-base font-extrabold tracking-tight ${m.status === "atingido" ? "text-accent-fg" : "text-fg"}`}
              >
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-1">
            <span className="text-[10.5px] font-semibold uppercase tracking-[0.13em] text-fg/55">
              {squad === "sdr" ? "SDR" : "Closer"} · valores do {rangeLabel} / meta do {rangeLabel}
            </span>
            <span className="font-display text-[28px] font-extrabold tracking-tight">{nome}</span>
          </div>
          <button type="button" onClick={onFechar} className="rounded-full border border-border-2 bg-glass-bg px-4 py-2 text-[12.5px] font-semibold">
            Fechar
          </button>
        </div>
          ...
          {pessoa.metricas.map((m) => {
            ...
            return (
              <div key={m.metrica} className="flex flex-col gap-1.5 rounded-xl border border-border-2 bg-bg-2/60 p-3.5">
                <span className="text-[10.5px] font-semibold uppercase tracking-[0.09em] text-fg/55">{m.nome_exibicao}</span>
                <div className="flex items-baseline justify-between gap-2">
                  <span
                    className={`font-display text-[19px] font-extrabold tracking-tight ${m.status === "atingido" ? "text-accent-fg" : "text-fg"}`}
                  >
                    {formatarNumero(m.realizado)} / {metaTexto(m)}
                  </span>
                  {estado === "ok" && <span className="text-[11.5px] font-semibold text-fg/60">{Math.round(pctMetrica(m))}%</span>}
                  {estado === "sem_meta" && <span className="text-[11.5px] font-semibold text-fg/50">sem meta</span>}
                </div>
                {estado === "ok" ? (
...
export function TimeComercialLista({ pessoas, granularidade }: Props) {
  ...
  if (pessoas.length === 0) {
    return <p className="text-sm text-fg/60">Nenhuma pessoa com dado lançado neste período.</p>;
  }

  return (
    <section className="flex flex-col gap-3">
      <div className="flex flex-wrap items-baseline justify-between gap-3.5">
        <span className="font-display text-xl font-semibold tracking-tight">Time comercial</span>
        <span className="text-[11.5px] text-fg/50">Clique em uma pessoa pra ver todas as métricas</span>
      </div>
