...
export function GaugeMeta({ pct, faltamLabel, caption }: GaugeMetaProps) {
  ...
  return (
    <article className="glass-panel flex flex-col gap-3 rounded-2xl px-[21px] pb-5 pt-[19px]">
      <span className="text-[17px] font-semibold uppercase tracking-[0.13em] text-fg/56">Atingimento da meta</span>
      <div className="flex flex-col items-center gap-0.5">
        ...
        <span className="font-display -mt-7 text-[50px] font-extrabold leading-none tracking-tight">
          {pct === null ? "—" : `${Math.round(pctClamp)}%`}
        </span>
        <span className="mt-1.5 text-center text-lg text-fg/55">{caption}</span>
      </div>
      <div className="flex justify-between border-t border-border-2 pt-[11px]">
        <span className="text-lg text-fg/55">Faltam</span>
        <span className="text-lg font-bold text-accent-fg">{faltamLabel}</span>
      </div>
    </article>
  );
}
